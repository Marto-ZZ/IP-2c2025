...
def contar_pacientes(registros:list[tuple[int,str]], enfermedad:str) -> int:
    cantidad_registros = 0
    for i in range(len(registros)):
        if enfermedad in registros[i][1]:
...
def alarma_epidemiologica(registros:list[tuple[int,str]], infecciosas:list[str], umbral: float) -> dict[str,float]:
    ...
    for enfermedad in infecciosas:
        for i in range(len(registros)):
            if enfermedad in registros[i][1] and enfermedad not in res:
                porcentaje = contar_pacientes(registros, enfermedad)/len(registros)
                if umbral < porcentaje:
                    res[enfermedad] = porcentaje
    return res

print(alarma_epidemiologica([(101, "Gripe"),(102, "Dengue"),(103, "Gripe"),(104, "Covid"),(105, "Dengue"),(106, "Gripe"),(107, "Migraña"),(108, "Covid"),(109, "Gripe"),(110, "Dengue")], ["Gripe", "Dengue"], 0.25))
...
def orden_de_atencion(urgentes:Cola[int], postergables:Cola[int]) -> Cola[int]:
    ...
    while not urgentes.empty():
        copia_urgentes.append(urgentes.get())
    while not postergables.empty():
        copia_postergables.append(postergables.get())

    for i in range(len(copia_urgentes)):
        urgentes.put(copia_urgentes[i])
        postergables.put(copia_postergables[i])
    
    for i in range(len(copia_urgentes)):
        res.put(copia_urgentes[i])
        res.put(copia_postergables[i])
    return res       
...
def nivel_de_ocupacion(camas_por_piso: list[list[bool]]) -> list[float]:
    ...
    for i in range(len(camas_por_piso)):
        camas_ocupadas = 0
        for j in range(cantidad_camas):
            if camas_por_piso[i][j] == True:
                camas_ocupadas += 1
        res.append(camas_ocupadas/cantidad_camas)
    return res

print(nivel_de_ocupacion([[True,True,False],[False,False,False],[True,True,True]]))
...
def sumar_horas(horas_empleado:list[int]) -> int:
    total = 0
    for i in range(len(horas_empleado)):
        total += horas_empleado[i]
    return total
...
def empleados_del_mes(horas:dict[int,list[int]]) -> list[int]:
    ...
    horas_max = [0]
    for i in horas.keys():
        if horas_max[0] < sumar_horas(horas[i]):
            horas_max.clear()
            horas_max.append(sumar_horas(horas[i]))
            res.clear()
            res.append(i)
        elif horas_max[0] == sumar_horas(horas[i]):
            res.append(i)
    return res

print(empleados_del_mes({101: [8, 10, 8], 102: [5, 5, 5], 103: [10, 8, 9], 104: [9, 9, 9]}))
